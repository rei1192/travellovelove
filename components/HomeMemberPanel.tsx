...
export default function HomeMemberPanel(){
  ...
  return <section className="homeMemberShell"><div className="memberDashboard"><div className="walletMini"><div><span>こんにちは</span><b>{profile.display_name||'TripNest Member'}</b></div><div><small>保有コイン</small><strong>● {profile.coin_balance.toLocaleString()}</strong><Link href="/coins" style={{display:'block',fontSize:9,marginTop:5,textDecoration:'none'}}>コインを追加 →</Link></div></div>{trip?<Link href="/reservations" className="nextTripCard"><div><span className="microLabel light">NEXT TRIP</span><h3>{trip.hotels?.name||'次の旅行'}</h3><p>{trip.check_in} → {trip.check_out} ・ {trip.total_nights}泊</p></div><span className="nextTripArrow">›</span></Link>:<div className="noTripCard"><div><span className="microLabel">NEXT TRIP</span><b>次の旅はまだ未定です</b><p>気になるホテルを保存して、旅の候補を作りましょう。</p></div><Link href="/saved">保存を見る</Link></div>}</div></section>
}
