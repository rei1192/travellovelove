...
export default function AuthPanel(){
 ...
 const client=sb
 async function signUp(){setMsg('処理中...');const{error}=await client.auth.signUp({email,password});setMsg(error?error.message:'確認メールを送信しました。')}
 async function signIn(){setMsg('処理中...');const{error}=await client.auth.signInWithPassword({email,password});setMsg(error?error.message:'ログインしました。')}
 async function signOut(){await client.auth.signOut();setMsg('ログアウトしました。')}
