...
export default function HotelExplorer(){
  ...
  return <>
    <section className="hero homeHero" id="search">
      ...
      <div className="heroContent"><div className="eyebrow">REAL HOTELS × SMART TRAVEL</div><h1>次の旅を、<br/>もっと直感的に。</h1><p>実在ホテルだけを、検索・保存・地図・予約までひとつのアプリで。</p>
      <div className="searchBox premiumSearch"><span>⌕</span><input value={q} onChange={e=>setQ(e.target.value)} placeholder="ホテル名・都市・エリア"/><button>検索</button></div>
      <div className="quickActions"><Link href="/map"><b>◉</b><span>地図から探す</span></Link><Link href="/saved"><b>♡</b><span>保存した宿</span></Link><Link href="/reservations"><b>✦</b><span>予約を見る</span></Link></div></div>
    </section>

    <section className="section discoverSection"><div className="sectionTitle"><div><div className="eyebrow">DISCOVER</div><h2>実在ホテルを探す</h2></div><span>{loading?'読込中':`${list.length}件`}</span></div>
      ...
      <div className="sortRow"><span>表示順</span><select value={sort} onChange={e=>setSort(e.target.value)}><option value="recommended">おすすめ順</option><option value="coin_asc">コインが少ない順</option><option value="rating">評価順</option></select></div>
      {loading?<div className="skeletonGrid"><div/><div/></div>:<div className="grid realHotelGrid">{list.map((h,i)=><article className="card realHotelCard" key={h.id}>
        <div className={'hotelVisual visual'+(i%4)}><div className="hotelVisualTop"><span className="realBadge">✓ 実在ホテル</span>{h.featured&&<span className="featured">人気</span>}</div><div className="hotelVisualBottom"><small>{h.prefecture}・{h.city}</small><strong>{h.name}</strong></div><button aria-label="お気に入り" className={'favBtn '+(favs.includes(h.id)?'liked':'')} onClick={()=>toggle(h.id)}>{favs.includes(h.id)?'♥':'♡'}</button></div>
        <div className="cardBody upgradedCardBody"><div className="ratingLine"><span>★ {Number(h.rating).toFixed(1)}</span><small>{h.review_count?`${h.review_count.toLocaleString()} reviews`:'Google Maps掲載'}</small></div><div className="amenityLine">{(h.amenities||[]).slice(0,3).map(a=><span key={a}>{a}</span>)}</div><div className="priceRow"><div className="price"><b>●</b> {h.coin.toLocaleString()} <small>coin / 泊〜</small></div><div className="cardLinks"><a href={h.google_maps_url||'#'} target="_blank" rel="noreferrer">地図</a><Link href={`/hotels/${h.slug}`} className="detailArrow">詳細 ›</Link></div></div></div>
      </article>)}</div>}
    </section>

    <section className="section mapPromo"><div><div className="eyebrow">MAP DISCOVERY</div><h2>場所で選ぶと、旅が見えてくる。</h2><p>Googleマップでホテルの場所を見ながら、エリア感や移動のしやすさまで確認できます。</p></div><Link href="/map" className="mapPromoButton">地図を開く →</Link></section>
  </>
}
