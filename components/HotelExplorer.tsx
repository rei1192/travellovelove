...
export default function HotelExplorer(){
  ...
  return <>
    <section className="hero homeHero" id="search">
      ...
      <div className="heroContent"><div className="eyebrow">CURATED STAYS × SMART TRAVEL</div><h1>次の旅を、<br/>もっと自由に。</h1><p>TripNestが厳選したホテルを、検索・保存・地図・予約までひとつの流れで楽しめます。</p>
    ...
    <section className="section discoverSection"><div className="sectionTitle"><div><div className="eyebrow">DISCOVER</div><h2>おすすめホテル</h2></div><span>{loading?'読み込み中':`${list.length}件`}</span></div>
      ...
      {loading?<div className="skeletonGrid"><div/><div/></div>:<div className="grid realHotelGrid">{list.map((h,i)=><article className="card realHotelCard" key={h.id}>
        <HotelPhoto placeId={h.google_place_id} name={h.name} className="hotelVisual" fallbackClass={'visual'+(i%4)}>
          <Link href={`/hotels/${h.slug}`} className="hotelVisualDetailLink" aria-label={`${h.name}の詳細を見る`}/>
          <div className="hotelVisualTop"><span className="realBadge">{h.featured?'✦ プレミアム':'おすすめ'}</span>{h.featured&&<span className="featured">人気</span>}</div>
          <div className="hotelVisualBottom"><small>{h.prefecture}・{h.city}</small><strong>{h.name}</strong><span className="tapHint">写真をタップして詳細へ</span></div>
          <button aria-label="お気に入り" className={'favBtn '+(favs.includes(h.id)?'liked':'')} onClick={(e)=>{e.preventDefault();e.stopPropagation();void toggle(h.id)}}>{favs.includes(h.id)?'♥':'♡'}</button>
        </HotelPhoto>
        <div className="cardBody upgradedCardBody"><div className="ratingLine"><span>★ {Number(h.rating).toFixed(1)}</span><small>{h.review_count?`口コミ ${h.review_count.toLocaleString()}件`:'注目のホテル'}</small></div><div className="amenityLine">{(h.amenities||[]).slice(0,3).map(a=><span key={a}>{a}</span>)}</div><div className="priceRow"><div className="price"><b>●</b> {h.coin.toLocaleString()} <small>コイン / 1泊〜</small></div><div className="cardLinks"><a href={h.google_maps_url||'#'} target="_blank" rel="noreferrer" className="mapLinkBtn" aria-label={`${h.name}を地図で見る`}>地図</a><Link href={`/hotels/${h.slug}`} className="detailCtaBtn" aria-label={`${h.name}の詳細を見る`}><span>詳細を見る</span><span className="detailCtaIcon">→</span></Link></div></div></div>
      </article>)}</div>}
    </section>
