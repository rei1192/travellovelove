  ...
  return <>
    ...
    <section className="section discoverSection"><div className="sectionTitle"><div><div className="eyebrow">DISCOVER</div><h2>おすすめホテル</h2></div><span>{loading?'読み込み中':`${list.length}件`}</span></div>
      ...
      {loading?<div className="skeletonGrid"><div/><div/></div>:<div className="grid realHotelGrid">{list.map((h,i)=><article className="card realHotelCard" key={h.id}>
        ...
        <div className="cardBody upgradedCardBody"><div className="ratingLine"><span>★ {Number(h.rating).toFixed(1)}</span><small>{h.review_count?`口コミ ${h.review_count.toLocaleString()}件`:'注目のホテル'}</small></div><div className="amenityLine">{(h.amenities||[]).slice(0,3).map(a=><span key={a}>{a}</span>)}</div><div className="priceRow"><div className="price"><b>●</b> {h.coin.toLocaleString()} <small>コイン / 1泊〜</small></div><div className="cardLinks"><a href={h.google_maps_url||'#'} target="_blank" rel="noreferrer" aria-label={`${h.name}を地図で見る`}>地図</a><Link href={`/hotels/${h.slug}`} className="detailArrow" aria-label={`${h.name}の詳細を見る`}>詳細を見る</Link></div></div></div>
      </article>)}</div>}
    </section>
