...
export default function HotelDetailPage(){
  ...
  return <main>
    <section className="detailHero premiumDetailHero"><Link href="/" className="backButton">‹</Link><button className="shareButton" onClick={share}>↗</button><div className="detailBadgeRow"><span className="realBadge">✓ 実在ホテル</span>{hotel.featured&&<span className="featured">人気</span>}</div><div className="detailOverlay"><span>★ {Number(hotel.rating).toFixed(1)} {hotel.review_count?`(${hotel.review_count.toLocaleString()}件)`:''}</span><h1>{hotel.name}</h1><p>{hotel.prefecture}・{hotel.city}・{hotel.area}</p></div></section>
    <section className="section detailSection"><div className="eyebrow">ABOUT</div><h2>このホテルについて</h2><p className="description">{hotel.description||'TripNest掲載の実在ホテルです。'}</p><div className="amenityGrid">{(hotel.amenities||[]).map(a=><div key={a}>✓ {a}</div>)}</div><GooglePlaceLive placeId={hotel.google_place_id}/></section>
