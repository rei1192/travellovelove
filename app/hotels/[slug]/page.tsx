...
export default function HotelDetailPage(){
  ...
  return <main>
    <section className="detailHero premiumDetailHero"><Link href="/" className="backButton">‹</Link><div className="detailBadgeRow"><span className="realBadge">✓ 実在ホテル</span>{hotel.featured&&<span className="featured">人気</span>}</div><div className="detailOverlay"><span>★ {Number(hotel.rating).toFixed(1)} {hotel.review_count?`(${hotel.review_count.toLocaleString()}件)`:''}</span><h1>{hotel.name}</h1><p>{hotel.prefecture}・{hotel.city}・{hotel.area}</p></div></section>
    <section className="section detailSection"><div className="eyebrow">ABOUT</div><h2>このホテルについて</h2><p className="description">{hotel.description||'TripNest掲載の実在ホテルです。'}</p><div className="amenityGrid">{(hotel.amenities||[]).map(a=><div key={a}>✓ {a}</div>)}</div></section>
    <section className="section mapDetailSection"><div className="sectionTitle"><div><div className="eyebrow">LOCATION</div><h2>Googleマップ</h2></div></div><div className="detailMap"><iframe title={`${hotel.name} map`} src={`https://www.google.com/maps?q=${mapQuery}&output=embed`} loading="lazy" referrerPolicy="no-referrer-when-downgrade"/></div><div className="addressCard"><div><b>{hotel.address||`${hotel.prefecture}${hotel.city}`}</b>{hotel.phone&&<span>{hotel.phone}</span>}</div><a href={hotel.google_maps_url||`https://www.google.com/maps/search/?api=1&query=${mapQuery}`} target="_blank" rel="noreferrer">Googleマップで開く ↗</a></div></section>
    <section className="bookingCard premiumBooking"><div className="bookingTop"><div><span>1泊あたり</span><strong>● {hotel.coin.toLocaleString()} coin〜</strong></div><span className="secureBadge">TripNest予約</span></div><div className="bookingFields"><label>チェックイン<input type="date" value={checkIn} onChange={e=>setCheckIn(e.target.value)}/></label><label>チェックアウト<input type="date" value={checkOut} onChange={e=>setCheckOut(e.target.value)}/></label><label>人数<select value={guests} onChange={e=>setGuests(Number(e.target.value))}>{[1,2,3,4,5,6].map(n=><option key={n}>{n}</option>)}</select></label></div>{nights>0&&<div className="bookingTotal"><span>{nights}泊・{guests}名</span><b>{total.toLocaleString()} coin</b></div>}<button className="primary" disabled={!checkIn||!checkOut||nights<1} onClick={reserve}>この内容で予約する</button>{msg&&<div className="message">{msg}</div>}<small className="finePrint">予約確定時にコインが差し引かれます。キャンセル時はコイン返金されます。</small></section>
  </main>
}
