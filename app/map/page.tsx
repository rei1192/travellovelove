...
export default function MapPage(){
  ...
  return <main>
    <section className="pageHero mapHero"><div className="eyebrow">GOOGLE MAPS</div><h1>地図からホテルを探す</h1><p>おすすめホテルの場所を確認しながら、経路や周辺スポットまでまとめてチェックできます。</p><div className="searchBox mapSearch"><span>⌕</span><input value={q} onChange={e=>setQ(e.target.value)} placeholder="ホテル名・都市で検索"/></div><div className="chips mapChips">{prefectures.map(x=><button key={x} className={'chip '+(pref===x?'active':'')} onClick={()=>setPref(x)}>{x}</button>)}</div></section>
    <section className="mapWorkspace">
      ...
      <div className="mapListHeader"><b>{list.length}件</b><span>ホテルを選択</span></div><div className="mapList">{list.map(h=><button key={h.id} onClick={()=>setSelected(h)} className={'mapHotel '+(selected?.id===h.id?'selected':'')}><div><span className="realBadge">{h.featured?'プレミアム':'おすすめ'}</span><small>{h.prefecture}・{h.city}</small><b>{h.name}</b><em>★ {Number(h.rating).toFixed(1)} ・ {h.coin.toLocaleString()} coin〜</em></div><span className="chev">›</span></button>)}</div>
    </section>
